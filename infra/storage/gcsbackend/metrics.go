...
// MetricsOption configures the gcsbackend metric constructor.
...
func WithRegisterer(reg prometheus.Registerer) MetricsOption {
	if reg == nil {
		panic("gcsbackend: WithRegisterer requires a non-nil registerer (omit the option for DefaultRegisterer)")
	}
	return func(c *metricsConfig) { c.registerer = reg }
}

// NewMetrics creates and registers GCS metrics. Pass [WithRegisterer]
// for a non-default registry.
func NewMetrics(opts ...MetricsOption) *Metrics {
	cfg := metricsConfig{registerer: prometheus.DefaultRegisterer}
	for _, opt := range opts {
		if opt == nil {
			panic("gcsbackend: NewMetrics option must not be nil")
		}
		opt(&cfg)
	}
	reg := cfg.registerer
