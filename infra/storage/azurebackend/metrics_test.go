...
func TestMetricsContract(t *testing.T) {
	...
	assertMetricLabels(t, reg, "storage_azure_operation_duration_seconds", []string{"operation", "storage_instance"})
	assertMetricLabels(t, reg, "storage_azure_operation_errors_total", []string{"operation", "storage_instance"})
...
func TestMetricsNormalizeExpectedNotFound(t *testing.T) {
	...
	// get must follow the same contract — a cache miss / probe must
	// not inflate operation_errors_total. Matches S3 / GCS / SFTP.
	metrics.observeOp("documents", "get", start, azureMetricErr(azureBlobNotFoundErr()))
