...
// target is the validated operator-chosen leadership key.
type Metrics struct {
...
func (m *Metrics) observeDrainDuration(d time.Duration, key, state string) {
	...
	m.drainDuration.WithLabelValues("redislock", key, state).Observe(d.Seconds())
}
...
func (m *Metrics) observeDrainWarn(key string) {
	...
	m.drainWarns.WithLabelValues("redislock", key).Inc()
}
