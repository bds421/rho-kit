...
func startNATS(t *testing.T) string {
	...
	c, err := nats.Run(ctx, "nats:2.11-alpine", nats.WithArgument("jetstream", ""))
...
func TestPublishConsume_RoundTrip(t *testing.T) {
	url := startNATS(t)
	conn, err := Connect(context.Background(), Config{URL: url})
	require.NoError(t, err)
	defer conn.Close()

	require.NoError(t, conn.EnsureStream(context.Background(), StreamConfig{
		Name:        "EVENTS",
	...
	cons := NewConsumer(conn, ConsumerConfig{
		Stream:        "EVENTS",
