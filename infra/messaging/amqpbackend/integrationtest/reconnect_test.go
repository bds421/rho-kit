...
func TestConnection_Reconnect_CallbackError_RetriesUntilCallbackSucceeds(t *testing.T) {
	container, url := startDedicatedRabbitMQ(t)
	...
	require.Eventually(t, func() bool {
		return callbackCalls.Load() >= 2 && conn.Healthy()
	}, 45*time.Second, 100*time.Millisecond)

	assert.GreaterOrEqual(t, callbackCalls.Load(), int32(2), "callback should be retried after an error")
}
