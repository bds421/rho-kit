...
// httpServerDirectConstructionRule flags direct construction of
// `net/http.Server` (composite literal `&http.Server{...}`,
// `http.Server{...}`, or `new(http.Server)`). The kit's
// `httpx.NewServer` wires `ReadHeaderTimeout`, `ReadTimeout`,
// `WriteTimeout`, `IdleTimeout`, `MaxHeaderBytes`, and a structured
// slog-backed `ErrorLog`. Bypassing it leaves the server vulnerable
// to slowloris-style resource exhaustion and routes connection
// errors through the global `log` package.
//
