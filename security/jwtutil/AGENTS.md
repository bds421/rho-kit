...
- `WithExpectedIssuer(iss)` / `WithExpectedAudience(aud)` — **MANDATORY for multi-service deployments**. Without them, a token issued for service A is silently valid at service B if they share a signer (confused-deputy attack, RFC 7519 §4.1.3).
- `WithAllowAnyIssuer()` / `WithAllowAnyAudience()` — explicit override; "Any" makes the relaxation auditable in code review.
...
- **No `WithExpectedAudience` in a multi-service deployment** — see above. The kit's verification will silently succeed when it should fail. Always set the audience.
