...
type BurnRateAlert struct {
	...
	// LongWindow is the longer evaluation window (e.g. 1h).
	LongWindow float64

	// ShortWindow is the shorter evaluation window (e.g. 5m).
	ShortWindow float64
...
func DefaultAlertThresholds() []BurnRateAlert {
	return []BurnRateAlert{
		{Severity: "critical", LongWindow: 1, ShortWindow: 5.0 / 60, BurnRateThreshold: 14.4},
		{Severity: "warning", LongWindow: 6, ShortWindow: 0.5, BurnRateThreshold: 6},
		{Severity: "info", LongWindow: 72, ShortWindow: 6, BurnRateThreshold: 1},
	}
}
