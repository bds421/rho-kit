...
func Module(jwksURL string, opts ...Option) app.Module {
	...
	if cfg.expectedIssuer == "" && !cfg.allowAnyIssuer {
		panic("app/jwt: pass WithIssuer(...) or WithoutIssuer() to acknowledge issuer policy")
	}
	if cfg.audience == "" && !cfg.allowAnyAudience {
		panic("app/jwt: pass WithAudience(...) or WithoutAudience() to acknowledge audience policy (RFC 7519 confused-deputy)")
	}
