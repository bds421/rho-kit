...
- **Module signature shape**: Every `app/<adapter>.Module(...)` follows one of two canonical shapes so codegen and AI agents can rely on it: `Module(cfg <Config>, opts ...Option)` when the adapter needs a structured config (postgres, nats, redis), or `Module(<resource> string, opts ...Option)` when a single URL/JWKS endpoint is enough (amqp, jwt). The first parameter is always the required input (named `cfg` for structs or named for the resource for strings); the variadic tail is always `opts ...Option`. Adapters that take no required input (tracing) drop the variadic. Never name the option tail anything other than `opts`.
- **Constructor return-type rule**: Constructors follow one of two patterns. `func New...(...) (*T, error)` is used by anything that allocates I/O resources, validates external input, or can fail at construction time (cache backends, stores hitting a network, anything that opens a pool). `func New...(...) *T` (no error) is used by in-memory primitives and metrics collectors that cannot fail — these instead `panic` on programmer errors (nil registerer, empty namespace) per the fail-fast convention above. The two patterns together mean: if you see an `error` return, you must check it; if you don't, programmer error is the only failure mode and it panics at construction.
- **`redact.WrapError` vs `fmt.Errorf("%w: ...", sentinel)`**: Use `redact.WrapError(prefix, err)` when wrapping an *opaque backend cause* whose `Error()` string might leak internals (driver errors, syscall errors, third-party SDK errors). Use plain `fmt.Errorf("%w: context", sentinel)` when annotating a *kit-owned sentinel* with descriptive context — the `%w` preserves `errors.Is` matching against the sentinel and there is nothing to redact. The two are not interchangeable: `redact.WrapError` strips the inner `Error()` text for log/response safety; sentinel wrapping keeps the chain navigable.
