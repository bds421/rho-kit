...
| RabbitMQ / AMQP | Use `amqpbackend.WithURLProvider` or `app.WithRabbitMQURLProvider`; the provider is called before each initial dial and reconnect. |
...
| SFTP | Use `SFTPConfig.PasswordProvider`; it is evaluated when a new SSH connection opens. Key files are read on each reconnect, so projected-key updates take effect after reconnect. |
