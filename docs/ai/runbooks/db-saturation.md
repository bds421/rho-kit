...
- Add a `lib/pq`-style query timeout on every connection so
  context-cancelled queries actually release their connection.
  rho-kit's `infra/sqldb` does this when callers thread a context.
