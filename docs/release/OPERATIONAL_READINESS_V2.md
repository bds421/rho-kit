...
| `github.com/bds421/rho-kit/examples/saga-coordinator/v2` | Example | Canonical multi-step transaction with compensation: runtime/saga.Run wrapped by data/idempotency cache + per-key exclusive section. Demonstrates roll-forward, automatic compensation in reverse on failure, idempotent retry safety, and the 422 vs 500 error-routing distinction. In-memory; production wires pgstore + pgadvisory + outbox. |
...
| `github.com/bds421/rho-kit/testing/integrationtest/v2` | Integration helper | Consolidated Docker-backed integration suites for all kit primitives (crypto, data, infra, observability). Replaces the 24 per-module `*/integrationtest/v2` submodules merged in Wave 154. |
