...
| Benchmark baselines captured | [benchmarks/v2.0.0/MANIFEST.md](benchmarks/v2.0.0/MANIFEST.md), `make bench-baseline` | Current benchmark suites have raw `go test -bench` outputs that can be used as `kit-bench-gate -baseline` inputs. | OPEN before tag: the checked-in 2026-05-12 files are historical/preliminary only and must not be used as canonical v2.0.0 `kit-bench-gate -baseline` inputs. Rerun `make bench-baseline` from a clean release-candidate commit and replace `docs/release/benchmarks/v2.0.0/` wholesale. |
...
| Dependency-aware release levels | `make release-plan`, `tools/plan-module-release.sh` | Internal `go.mod` requires are converted to dependency levels so modules that can be tagged together are explicit. Changed-mode reports modules changed since a base ref plus impacted dependents. | Passed 2026-05-14 (refreshed after waves 60/61): `RELEASE_MODE=all make release-plan` reports 77 modules across six dependency levels (7/8/7/26/23/6) including `observability/auditlog/postgres` and `infra/outbox/postgres`. |
...
| Local release rehearsal | `tools/rehearse-v2-release.sh` | Temporary clone and bare origin prove dependency-ordered tags, level tidies, downstream `go get`, `go.sum`, and command installs without touching real origin. | Passed 2026-05-13 on the 73-module tree (6 dependency levels: 7/8/6/25/21/6). Downstream consumer resolved every module at `v2.0.0` and command installs of `cmd/kit-new`, `cmd/kit-migrate`, and `cmd/kit-doctor` succeeded. Log written to `docs/release/rehearsals/20260513T050809Z-v2-release-rehearsal.log` (gitignored as a local evidence artifact). The earlier 67-module rehearsal entry was replaced after the workspace added six modules between rehearsal and re-validation. |
...
| Future multi-module tag plan exists | [TAGGING_PLAN_V2.md](TAGGING_PLAN_V2.md) | Exact dependency-ordered tag strategy, commands, expected level count, and rollback are documented without creating tags now. | Refreshed 2026-05-14 after waves 60/61: plan creates 77 module-prefixed tags across six dependency levels (level 0–5) plus `release/v2.0.0` coordination tag later, matching the current `RELEASE_MODE=all make release-plan` output. |
...
All commands above passed. The earlier API-freeze recheck claim ("`go list -m`
against `API_FREEZE_V2.md` produced no missing or extra modules") was a false
positive: in workspace mode `go list -m` prints every module's path, so the
grep collapsed to a tautology. Wave 65 replaced that ad-hoc check with
...
## Metric contract polish closure: M-004 / M-008

The 2026-05-13 hostile review's metric contract findings are CLOSED for
v2.0.0:
