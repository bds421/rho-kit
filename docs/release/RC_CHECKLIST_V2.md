...
external dependencies. The current local workspace still carries unrelated
module tidy edits; the live workspace `make check-dependency-allowlist` reports
`runtime/temporal/go.mod: github.com/nexus-rpc/sdk-go` until that separate
dirty change is either committed with allowlist review or removed.
