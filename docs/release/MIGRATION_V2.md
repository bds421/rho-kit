...
| Scope limits | The initial wave deliberately shipped a narrow surface. Subsequent waves removed every entry on that list while preserving the wave-128 API: wave 161 added multiple content types per status (`WithResponseContentT[T]`, `WithResponseContent`) plus response headers (`WithResponseHeader`); wave 162 added auto-discovery of path parameters from the OAS `{name}` template (opt-out via `WithSkipPathParamDiscovery`); wave 163 added per-operation `externalDocs` (`WithExternalDocs`), request/response/parameter examples (`WithRequestExample`, `WithResponseExample`, `WithParameterExample`), and `Tag.ExternalDocs` emission. The pre-161 single-content, no-example, no-externalDocs registrations continue to compile and emit identical JSON. |
...
| `crypto/envelope/awskms` Prometheus metric | Wave 184: the v1-era `kit_awskms_request_errors_total` series was renamed to `awskms_request_errors_total` so the awskms adapter follows the kit-wide `Namespace=<domain>` convention (no other kit metric carries a `kit_` prefix). Update Prometheus alert rules, recording rules, and Grafana panels that reference the old name. Label set (`code`, `operation`) is unchanged. |
