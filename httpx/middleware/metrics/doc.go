...
//
// It emits `http_requests_total{method,path,status}`,
// `http_request_duration_seconds{method,path}`, and
// `http_requests_in_flight`. New services should prefer the v2 module
// [github.com/bds421/rho-kit/observability/v2/redmetrics], which spells
// the route label `route`, separates errors into a `*_errors_total`
// counter, and offers namespace/subsystem options for multi-service
...
//     against the v1 label names; renaming would silently void them.
//     The package is preserved without breaking changes so legacy
